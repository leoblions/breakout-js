...
export function collideRect (rect1, rect2) {
  ...
  if (rect1.x > r2x2 || rect2.x > r1x2 || rect1.y > r2y2 || rect2.y > r1y2) {
    return false
  } else {
    return true
  }
...
export function withinRange (known, test, radius) {
  if (Math.abs(known - test) < radius || Math.abs(test - known) < radius) {
    return true
  } else {
    return false
  }
